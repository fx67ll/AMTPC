...
export class ParticlePool {
  ...
  initPointsSystem() {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      this.lifetimes[i] = 0;
      this.decays[i] = 0.01;
      this.gravities[i] = 0.05;
      this.rotationSpeeds[i] = 0;

      // 初始位置设置为(0,0,0)而不是屏幕外
  ...
  releaseParticleByIndex(index) {
    ...
    this.decays[index] = 0.01;
    this.gravities[index] = 0.05;
  ...
  setParticlePosition(index, x, y, z) {
    if (index < 0 || index >= this.maxParticles) return;
  ...
  update(deltaTime = 0.016) {
    ...
    usedIndices.forEach(index => {
      ...
      // 更新生命周期
      this.lifetimes[index] -= this.decays[index] * dt * 60; // 调整为每秒衰减
      ...
      // 应用重力
      if (this.gravities[index] && this.gravities[index] > 0) {
        velocity.y -= this.gravities[index] * dt;
      }
